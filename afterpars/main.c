...
int calculate_shaded_wall_color(t_game *game)
{
    ...
    float brightness = 1.0 - (game->wall->wall_distance / max_distance);
    if (brightness < 0.3)
        brightness = 0.3;
    if (brightness > 1.0)
        brightness = 1.0;
    ...
    // Apply brightness
    int r = ((base_color >> 16) & 0xFF) * brightness;
    int g = ((base_color >> 8) & 0xFF) * brightness;  
    int b = (base_color & 0xFF) * brightness;
