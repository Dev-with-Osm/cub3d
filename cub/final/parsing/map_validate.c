...
/*   Updated: 2025/12/30 10:13:50 by okhourss         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ff_alloc(t_ff *f, char **map, int w, int h)
{
	...
	f->vis = (unsigned char *)ft_calloc((size_t)f->ww * f->hh, 1);
	...
	if (!f->grid || !f->vis || !f->qx || !f->qy)
		return (1);
	ft_memset(f->grid, ' ', (size_t)f->ww * f->hh);
	while (i < h)
	{
		ft_memcpy(f->grid + (i + 1) * f->ww + 1, map[i], (size_t)w);
		i++;
	}
	return (0);
}
