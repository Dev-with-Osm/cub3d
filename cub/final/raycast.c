...
/*   Updated: 2025/12/30 16:56:14 by hoel-mos         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	init_ray(t_game *game, float angle, t_dda *dda)
{
	...
	{
		dda->step_x = -1;
		dda->side_dist_x = (game->map->player_x - dda->map_x) * dda->delta_dist_x;
	}
	else
	{
		dda->step_x = 1;
		dda->side_dist_x = (dda->map_x + 1.0f - game->map->player_x) * dda->delta_dist_x;
	}
	if (dda->ray_dir_y < 0)
	{
		dda->step_y = -1;
		dda->side_dist_y = (game->map->player_y - dda->map_y) * dda->delta_dist_y;
	}
	else
	{
		dda->step_y = 1;
		dda->side_dist_y = (dda->map_y + 1.0f - game->map->player_y) * dda->delta_dist_y;
	}
}
